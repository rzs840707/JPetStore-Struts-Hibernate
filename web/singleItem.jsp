...
<div class="w" style="margin-top: 120px;">
    ...
    <div itemid="<%=item.getItemid()%>" productid="<%=item.getProductid()%>">
        ...
        <div class="photo—box">
            <img src="<%="image/"+item.getImage()%> " title="<%=item.getName()%>" onerror="" style="border-radius: 100%;box-shadow: 1px 3px 20px 2px #801414;">
        </div>
        <h2>
            <span>
                <%=item.getName()%>
        ...
        <div class="buy-it">
            ...
            <P>
                <s:text name="Qty"></s:text>:<br><input id="<%=item.getItemid()%>+quatity" type="number" min="1"
            ...
            <a id="<%=item.getItemid()%>+addToCart"
               href="orders_makeOrder?purchase=0&orders.quatity=1&orders.itemid=<%=item.getItemid()%>"><button
                    style="" onclick="//没用//location.href ='orders_makeOrder?purchase=-1&orders.itemid=';"><s:text name="AddToCart"></s:text></button></a>
