...
public class OrderAction extends ActionSupport {
    ...
    private Map<String,String> resultInfo; //实现ajax一定要设置get方法不然传不出去
...
//        }
        ...
        if(purchase.equals("-1")){ //直接进入购物车
            ...
            ((Map)ActionContext.getContext().get("request")).put("totalValue", totalValue);
            ((Map)ActionContext.getContext().get("request")).put("cart", ordersDAO.getAllOders(ordersid));
            for (Orders order:ordersDAO.getAllOders(ordersid)
                 ) {
                itemNameMap.put(order.getItemid(),itemDAO.getItemByItemId(order.getItemid()).getName());
            }
            ((Map)ActionContext.getContext().get("request")).put("itemNameList", itemNameMap);
            System.out.println(itemNameMap);
    ...
    public String purchase() throws Exception{
        ...
        resultInfo.put("flag","谢谢您完成本次订单，共消费$"+totalPrice);
