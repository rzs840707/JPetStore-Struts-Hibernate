...
public class UserDAOJdbcImpl extends DAO<User> implements UserDAO {
    ...
    public void update(User user) {
        String sql="update user set password=? ,phone=?,email=?,firstname=?,lastname=?,address=?,zip=? where userid=?";
        update(sql, user.getPassword(), user.getPhone(), user.getEmail(), user.getFirstname(), user.getLastname(), user.getAddress(),  user.getZip(),user.getUserid() );
    }
